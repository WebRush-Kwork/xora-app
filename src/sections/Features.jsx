			...
			<Element name='features'>
				<div className='container'>
					<div className='relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3'>
						{features.map(({ id, text, icon, button, caption, title }) => (
							<div
								key={id}
								className='relative flex-50 max-md:g7 pb-5 max-md:pb-10 md:px-10 max-md:px-5 max-md:border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex-320'
							>
								...
								<h2 className='max-w-400 mb-7 h3 text-p4 max-md:mb-6 max-md:h5 max-md:my-6'>
									{title}
								</h2>
						...
						<ul className='relative flex justify-around flex-grow border-2 border-s3 rounded-7xl max-md:hidden'>
							...
							{details.map(({ id, icon, title }) => (
								<li key={id} className='relative pt-16 px-4 pb-14'>
									...
									<h3 className='text-center uppercase base-bold relative z-2 max-w-36 mx-auto my-0'>{title}</h3>
								</li>
							))}
						</ul>
					</div>
